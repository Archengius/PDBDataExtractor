...
DiaChildSymbolIterator::DiaChildSymbolIterator( const CComPtr<IDiaSymbol>& InParentSymbol, enum SymTagEnum InSymbolType, const WCHAR* InName, DWORD CompareFlags )
{
    ...
    const HRESULT GetCountResult = UnderlyingEnumerator->get_Count( &ItemCount );
    assert( SUCCEEDED( GetCountResult ) );
}
...
CComPtr<IDiaSymbol> DiaChildSymbolIterator::operator*() const
{
    ...
    const HRESULT ItemRetrievalResult = UnderlyingEnumerator->Item( CurrentIndex, &NextSymbol );
    assert( SUCCEEDED( ItemRetrievalResult ) );
    return NextSymbol;
}
